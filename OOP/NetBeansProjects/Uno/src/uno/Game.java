...
public class Game {
    ...
    public void start(Game game) {
        ...
        if (card.getValue() == UnoCard.Value.Skip) {
            JLabel message = new JLabel(playerIds[currentPlayer] + " Passou! ");
        ...
        if (card.getValue() == UnoCard.Value.Reverse) {
            JLabel message = new JLabel(playerIds[currentPlayer] + " A direção do jogo mudou! ");
            message.setFont(new Font("Arial", Font.BOLD, 48));
            JOptionPane.showMessageDialog(null, message);
            gameDirection ^= true;
            currentPlayer = playerIds.length - 1;
    ...
    public int getPlayerHardSize(String pid) {
    ...
    public void checkPlayerTurn(String pid) throws InvalidPlayerTurnException {
        if (this.playerIds[this.currentPlayer] != pid) {
            throw new InvalidPlayerTurnException(" Não é o seu turno " + pid, pid);
        }
    }
    ...
    public void submitPlayerCard(String pid, UnoCard card, UnoCard.Color declaredColor)
        ...
        if (!validCardPlay(card)) {
            ...
            if (card.getColor() != validColor) {
                JLabel message = new JLabel(" Movimento inválido, se espera que jogue a cor: " + validColor + " não pode usar a cor " + card.getColor());
                message.setFont(new Font("Arial", Font.BOLD, 48));
                JOptionPane.showMessageDialog(null, message);
                throw new InvalidColorSubmissionException(" Movimento inválido, se espera que jogue a cor: " + validColor + " não pode usar a cor " + card.getColor(), card.getColor(), validColor);
            } else if (card.getValue() != validValue) {
                JLabel message2 = new JLabel(" Movimento inválido, se espera que jogue o valor: " + validValue + " não pode usar o valor " + card.getValue());
                message2.setFont(new Font("Arial", Font.BOLD, 48));
                JOptionPane.showMessageDialog(null, message2);
                throw new InvalidValueSubmissionException(" Movimento inválido, se espera que jogue o valor: " + validValue + " não pode usar o valor " + card.getValue(), card.getValue(), validValue);
            }
        }

        pHand.remove(card);
        if (hasEmptyHand(this.playerIds[currentPlayer])) {
            JLabel message2 = new JLabel(this.playerIds[currentPlayer] + " Ganhou o jogo! Obrigado pro jogar!! ");
            message2.setFont(new Font("Arial", Font.BOLD, 48));
            JOptionPane.showMessageDialog(null, message2);
            System.exit(0);
        }
        ...
        if (card.getValue() == UnoCard.Value.DrawTwo) {
            ...
            getPlayerHand(pid).add(deck.drawCard());
            getPlayerHand(pid).add(deck.drawCard());
            JLabel message = new JLabel(pid + " Pegou 2 cartas!");
        }
        if (card.getValue() == UnoCard.Value.DrawTwo) {
            pid = playerIds[currentPlayer];
            getPlayerHand(pid).add(deck.drawCard());
            ...
            getPlayerHand(pid).add(deck.drawCard());
            getPlayerHand(pid).add(deck.drawCard());
            JLabel message = new JLabel(pid + " Pegou 4 cartas!");
        }
        if (card.getValue() == UnoCard.Value.Skip) {
        ...
        if (card.getValue() == UnoCard.Value.Reverse) {
            JLabel message = new JLabel(pid + " Usou Reverse!");
            message.setFont(new Font("Arial", Font.BOLD, 48));
            JOptionPane.showMessageDialog(null,
                    message);
